...
def test_forgot_password_page(browser):
    ...
    WebDriverWait(browser, 25).until(EC.presence_of_element_located(NewPassLocators.NEWPASS_CAPTCHA))
    page.btn_click_continue()

    WebDriverWait(browser, 30).until(
        EC.presence_of_element_located((By.XPATH, "//a[contains(text(), 'получение почты.')]"))
    )
    ...
    WebDriverWait(browser, 30).until(EC.visibility_of_element_located(NewPassLocators.NEWPASS_ONETIME_CODE))
	
    for i in range(0, 6):
        browser.find_elements(*NewPassLocators.NEWPASS_ONETIME_CODE)[i].send_keys(reg_code[i])
        WebDriverWait(browser, 5).until(EC.visibility_of_element_located(NewPassLocators.NEWPASS_ONETIME_CODE))
		
        WebDriverWait(browser, 10).until(EC.visibility_of_element_located(NewPassLocators.NEWPASS_NEW_PASS))
    ...
    def input_new_pass(new_pass):
        WebDriverWait(browser, 10).until(EC.element_to_be_clickable(NewPassLocators.NEWPASS_NEW_PASS)).clear()
        elem_new_pass.send_keys(Keys.COMMAND, 'a')
        elem_new_pass.send_keys(Keys.DELETE)
        elem_new_pass.send_keys(new_pass)
        WebDriverWait(browser, 10).until(EC.visibility_of_element_located(NewPassLocators.NEWPASS_NEW_PASS_CONFIRM)).clear()
        elem_conf_pass.send_keys(Keys.COMMAND, 'a')
        elem_conf_pass.send_keys(Keys.DELETE)
        elem_conf_pass.send_keys(new_pass)
        WebDriverWait(browser, 10).until(EC.invisibility_of_element_located(AuthLocators.LOADING_SPINNER))
